...
export function DebugPane() {
    ...
    (async () => {
      if (image && image !== analyzedImageRef.current) {
        ...
        try {
          ...
          const response =
            await ScreenWatcher.instance.getScreenshotDescriptionLocal(image, abortControllerRef.current);
          // check if stale
  ...
  if (activity && row) {
    ...
    innerElement = (
      <div
        className={[
          "absolute overflow-scroll top-10 left-0 flex flex-col items-end justify-end w-96 max-w-96 bg-black bg-opacity-80 p-4 rounded-xl m-2 text-white text-sm font-mono h-[80vh]",
          classes,
        ].join(" ")}
        ref={ref}
        style={style}
